...
export async function createGroupForJob(jobId: number, groupName: string) {
    try {
        // グループ名の重複チェック
        const isNameAvailable = await GroupService.isGroupNameAvailable(groupName, jobId)
        if (!isNameAvailable) {
 ...
 */
export async function createAndJoinWaitingRoom(jobId: number, groupName: string) {
    try {
        // グループ名の重複チェック
        const isNameAvailable = await GroupService.isGroupNameAvailable(groupName, jobId)
        if (!isNameAvailable) {
