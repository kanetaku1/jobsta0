    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 text-center">
                <h2 className="text-2xl font-semibold mb-2">{group.name || `グループ #${group.id}`}</h2>
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                ...
                <div className="grid grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <p className="font-medium text-blue-600">{group.job!.wage.toLocaleString()}円</p>
                    </div>
                    <div>
                        <p className="text-sm text-gray-500">勤務日</p>
                        <p className="font-medium">{new Date(group.job!.jobDate).toLocaleDateString('ja-JP')}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 text-center">
                    <p className="text-2xl font-bold text-blue-600">{group.members?.length || 0}</p>
                    <p className="text-gray-600">参加者数</p>
                </div>
                <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 text-center">
                    <p className="text-2xl font-bold text-green-600">{group.applications?.length || 0}</p>
                    <p className="text-gray-600">応募者数</p>
                </div>
                <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 text-center">
                    <p className="text-2xl font-bold text-purple-600">
                        {group.members && group.applications && group.members.length > 0 ? 
                            Math.round((group.applications.length / group.members.length) * 100) : 0}%
                    </p>
                ...
                {group.applications && group.applications.length > 0 ? (
                    <div className="space-y-3">
                        {group.applications.map((application) => (
                            <div key={application.id} className="flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-200">
                                <div>
                                    <p className="font-medium">応募者 #{application.id}</p>
                                    <p className="text-sm text-gray-600">
                                        応募日: {new Date(application.submittedAt).toLocaleDateString('ja-JP')}
                                    </p>
                                </div>
